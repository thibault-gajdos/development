...
:Options_R:
#+property: session *R*
#+property: header-args:R+ :session :tangle yes
...
#+BEGIN_SRC R  :results silent :exports none :tangle yes
    rm(list=ls(all=TRUE))  ## efface les données
    source('~/thib/projects/tools/R_lib.r')
    setwd('~/thib/projects/developpement/simon2/')
...
We use  half normal(0,1) as priors (uninformative). 

#+BEGIN_SRC R  :results output silent :exports code :tangle yes  :session
...
#+BEGIN_SRC R  :results output  :exports results :tangle yes  :session
